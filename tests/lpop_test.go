...
func setupLPopTestClient() *client.Client {
	conn1, _ := net.Pipe()
	...
	tests := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T) {
				...
				if val.ListData[0] != "two" {
					t.Errorf("Expected first element to be 'two', got %s", val.ListData[0])
				}
			},
		},
		{
